...
TMP_DIR=/path/to/${SAMPLE_ID}_cutesv
TMP_LOG=${TMP_DIR}/sv_call.${SAMPLE_ID}_pass.vcf.log

CUTESV_VERSION=`cuteSV --version | awk '{print "v"$2}'`
DATE=`date "+%Y%m%d"`
OUT_DIR=/path/to/sv_calls/cutesv/${CUTESV_VERSION}/${SAMPLE_ID}/${DATE}
...
REFERENCE=/path/to/chm13_reference_dir/chm13_v1.1_with_HG002_chrY.masked_PARs.fasta
